...
/**
 * REST controller for managing {@link ar.edu.um.gestioneventos.domain.Asiento}.
 */
@RestController
...
public class AsientoResource {
    ...
    public ResponseEntity<List<AsientoDTO>> getAllAsientos(
        ...
        Page<AsientoDTO> page;
        if (eagerload) {
            page = asientoService.findAllWithEagerRelationships(pageable);
        } else {
            page = asientoService.findAll(pageable);
        }
