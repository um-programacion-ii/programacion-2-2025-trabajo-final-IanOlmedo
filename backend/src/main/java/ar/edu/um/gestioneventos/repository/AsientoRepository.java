...
public interface AsientoRepository extends JpaRepository<Asiento, Long> {
    ...
    @Query(
        value = "select asiento from Asiento asiento left join fetch asiento.evento_con_asientos",
        countQuery = "select count(asiento) from Asiento asiento"
    )
    Page<Asiento> findAllWithToOneRelationships(Pageable pageable);

    @Query("select asiento from Asiento asiento left join fetch asiento.evento_con_asientos")
    List<Asiento> findAllWithToOneRelationships();

    @Query("select asiento from Asiento asiento left join fetch asiento.evento_con_asientos where asiento.id =:id")
