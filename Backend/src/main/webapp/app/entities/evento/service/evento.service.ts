...
type RestOf<T extends IEvento | NewEvento> = Omit<T, 'fecha' | 'ultimaActualizacion'> & {
  fecha?: string | null;
...
export class EventoService {
  ...
  protected convertDateFromClient<T extends IEvento | NewEvento | PartialUpdateEvento>(evento: T): RestOf<T> {
    return {
      ...
      ultimaActualizacion: evento.ultimaActualizacion?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvento: RestEvento): IEvento {
    return {
      ...
      ultimaActualizacion: restEvento.ultimaActualizacion ? dayjs(restEvento.ultimaActualizacion) : undefined,
    };
  }
